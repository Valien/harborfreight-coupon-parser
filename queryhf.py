...
def get_images(moyr, numimages):
    ...
    for n in range(numimages):
        ...
        '''    
        if n < 10:
            urltoparse = BaseURL + moyr + '/images/' + str(n).zfill(2) + ".jpg"
            print(urltoparse)
            response = requests.get(urltoparse)  # + str(n))
            if response.status_code == 200:
                # to do: remove my hard-coded desktop location...
                with open("/Users/allen.vailliencourt/Desktop/hf/" + moyr + '-' + str(n) + '.jpg', 'wb') as f:
...
my_input = input('Enter Month and Year in this format - july2019, etc.: ')
